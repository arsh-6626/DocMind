...
from type import Text, Caption, Figure, Equation, Headline, RichContent
...
def _generate_audio_and_caption_elevenlabs(
    ...
    """
    ELEVENLABS_API_KEY = "sk_042e6d1c0cd8e1a86464b6f1e7b286d77797ea9c3e3ecf70"
            ...
            match script_content:
                ...
                case Text(content=content, audio=None, captions=None):
                    ...
                    and os.uname().machine in ('arm64', 'aarch64')):
                        ...
                        audio_numpy, sr = sf.read(audio_path)
                        audio = torch.tensor([audio_numpy.T])
...
def _generate_audio_lmnt(content: str, output_file: str) -> dict:
    LMNT_API_KEY = "d8c651c3b1064b0792322a1b241993ed"
...
def fill_rich_content_time(script_contents: list[RichContent | Text]) -> list[RichContent | Text]:
    ...
    while k < len(script_contents):
        ...
        if not next_text_group or not current_rich_content_group:
            continue
            
        # Add null checks here
        start_time = next_text_group[0].start if next_text_group[0].start is not None else 0.0
        end_time = next_text_group[-1].end if next_text_group[-1].end is not None else start_time + 1.0
        
        total_duration = end_time - start_time
        
        # Avoid division by zero
        if len(current_rich_content_group) > 0:
            duration_per_rich_content = total_duration / len(current_rich_content_group)
        else:
            duration_per_rich_content = 1.0  # Default value if no rich content
